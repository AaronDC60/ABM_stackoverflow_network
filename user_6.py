...
class user:
    ...
    def eval(self):
        ...
        for q in self.my_questions[::-1]:
            ...
            if q.age == 2:
                ...
                self.p_ask = self.update_p(self.p_ask, q.n_upvotes, self.upvote_bias)
                self.p_active = self.update_p(self.p_active, q.n_upvotes, self.upvote_bias)
                ...
                if q.answers:
                    ...
                    for a in q.answers:
                        ...
                        user.p_answer = self.update_p(user.p_answer, a.n_upvotes, user.upvote_bias)
                        user.p_active = self.update_p(user.p_active, a.n_upvotes, user.upvote_bias)
    ...
    def step(self):
        ...
        for q in self.vis_questions:
            ...
            if not answered:
                q.answers.sort(key=lambda x: x.n_upvotes, reverse=True)
                # Upvote answers
                for a in q.answers:
...
class question:
    ...
    n_upvotes : int
        number of upvotes the question has received
    answers : list
...
class answer:
    ...
    n_upvotes : int
        number of upvotes the answer has received    
    """
