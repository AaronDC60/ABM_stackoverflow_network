...
class user:
    ...
    def __init__(self, system, i, tag):
        ...
        self.p_active = norm.cdf(np.random.normal())
    ...
    def eval(self):
        ...
        for q in self.my_questions[::-1]:
            ...
            if q.age == 2:
                ...
                # Update probability of being active
                x = np.log(self.p_active/(1-self.p_active))
                change = q.n_upvotes - self.upvote_bias
                if change < -5:
                    change = -5
                elif change > 5:
                ...
                if q.answers:
                    ...
                    for a in q.answers:
                        ...
                        # Update probability of being active
                        x = np.log(self.system.users[a.responder].p_active/(1-self.system.users[a.responder].p_active))
                        change = a.n_upvotes - self.system.users[a.responder].upvote_bias
                        if change < -5:
                            change = -5
                        elif change > 5:
                            change = 5
                        x += (0.1 * change)
                        self.system.users[a.responder].p_active = 1 / (1 + np.exp(-x))
